...
public class ReLogoObserver extends BaseObserver{
	...
	/**
	 * Makes a number of randomly oriented gardeners and then executes a set of commands on the
	 * created gardeners.
	 * 
	 * @param number
	 *            a number
	...
	public AgentSet<garden.relogo.Gardener> createGardeners(int number, Closure closure) {
		AgentSet<garden.relogo.Gardener> result = new AgentSet<>();
		AgentSet<Turtle> createResult = this.crt(number,closure,"Gardener");
	 ...
	 */
	@ReLogoBuilderGeneratedFor("garden.relogo.Gardener")
	public AgentSet<garden.relogo.Gardener> createGardeners(int number) {
		return createGardeners(number,null);
	}

	/**
	 * Makes a number of uniformly fanned gardeners and then executes a set of commands on the
	 * created gardeners.
	 * 
	 * @param number
	 *            a number
	 ...
	 */
	@ReLogoBuilderGeneratedFor("garden.relogo.Gardener")
	public AgentSet<garden.relogo.Gardener> createOrderedGardeners(int number, Closure closure) {
		AgentSet<garden.relogo.Gardener> result = new AgentSet<>();
		AgentSet<Turtle> createResult = this.cro(number,closure,"Gardener");
		for (Turtle t : createResult){
			if (t instanceof garden.relogo.Gardener){
				result.add((garden.relogo.Gardener)t);
			}
	 ...
	 */
	@ReLogoBuilderGeneratedFor("garden.relogo.Gardener")
	public AgentSet<garden.relogo.Gardener> createOrderedGardeners(int number) {
		return createOrderedGardeners(number,null);
	}
	...
	public AgentSet<garden.relogo.Gardener> gardeners(){
		...
		for (Object e : this.getContext().getObjects(garden.relogo.Gardener.class)) {
	...
	public AgentSet<garden.relogo.Gardener> gardenersOn(Patch p){
		...
		for (Turtle t : Utility.getTurtlesOnGridPoint(p.getGridLocation(),this,"gardener")){
			if (t instanceof garden.relogo.Gardener)
			result.add((garden.relogo.Gardener)t);
		}
		return result;
	...
	public AgentSet<garden.relogo.Gardener> gardenersOn(Turtle t){
		AgentSet<garden.relogo.Gardener> result = new AgentSet<garden.relogo.Gardener>();						
		for (Turtle tt : Utility.getTurtlesOnGridPoint(Utility.ndPointToGridPoint(t.getTurtleLocation()),this,"gardener")){
	...
	/**
	 * Makes a number of randomly oriented plants and then executes a set of commands on the
	 * created plants.
	 * 
	 * @param number
	 *            a number
	...
	public AgentSet<garden.relogo.Plant> createPlants(int number, Closure closure) {
		AgentSet<garden.relogo.Plant> result = new AgentSet<>();
		AgentSet<Turtle> createResult = this.crt(number,closure,"Plant");
		for (Turtle t : createResult){
	 ...
	 */
	@ReLogoBuilderGeneratedFor("garden.relogo.Plant")
	public AgentSet<garden.relogo.Plant> createPlants(int number) {
		return createPlants(number,null);
	}

	/**
	 * Makes a number of uniformly fanned plants and then executes a set of commands on the
	 * created plants.
	 * 
	 * @param number
	 *            a number
	 ...
	 */
	@ReLogoBuilderGeneratedFor("garden.relogo.Plant")
	public AgentSet<garden.relogo.Plant> createOrderedPlants(int number, Closure closure) {
		AgentSet<garden.relogo.Plant> result = new AgentSet<>();
		AgentSet<Turtle> createResult = this.cro(number,closure,"Plant");
		for (Turtle t : createResult){
			if (t instanceof garden.relogo.Plant){
				result.add((garden.relogo.Plant)t);
			}
	 ...
	 */
	@ReLogoBuilderGeneratedFor("garden.relogo.Plant")
	public AgentSet<garden.relogo.Plant> createOrderedPlants(int number) {
		return createOrderedPlants(number,null);
	}
	...
	public AgentSet<garden.relogo.Plant> plants(){
		...
		for (Object e : this.getContext().getObjects(garden.relogo.Plant.class)) {
	...
	public AgentSet<garden.relogo.Plant> plantsOn(Patch p){
		...
		for (Turtle t : Utility.getTurtlesOnGridPoint(p.getGridLocation(),this,"plant")){
			if (t instanceof garden.relogo.Plant)
			result.add((garden.relogo.Plant)t);
		}
		return result;
	...
	public AgentSet<garden.relogo.Plant> plantsOn(Turtle t){
		AgentSet<garden.relogo.Plant> result = new AgentSet<garden.relogo.Plant>();						
		for (Turtle tt : Utility.getTurtlesOnGridPoint(Utility.ndPointToGridPoint(t.getTurtleLocation()),this,"plant")){
	...
	/**
	 * Makes a number of randomly oriented rabbits and then executes a set of commands on the
	 * created rabbits.
	 * 
	 * @param number
	 *            a number
	...
	public AgentSet<garden.relogo.Rabbit> createRabbits(int number, Closure closure) {
		AgentSet<garden.relogo.Rabbit> result = new AgentSet<>();
		AgentSet<Turtle> createResult = this.crt(number,closure,"Rabbit");
		for (Turtle t : createResult){
	 ...
	 */
	@ReLogoBuilderGeneratedFor("garden.relogo.Rabbit")
	public AgentSet<garden.relogo.Rabbit> createRabbits(int number) {
		return createRabbits(number,null);
	}

	/**
	 * Makes a number of uniformly fanned rabbits and then executes a set of commands on the
	 * created rabbits.
	 * 
	 * @param number
	 *            a number
	 ...
	 */
	@ReLogoBuilderGeneratedFor("garden.relogo.Rabbit")
	public AgentSet<garden.relogo.Rabbit> createOrderedRabbits(int number, Closure closure) {
		AgentSet<garden.relogo.Rabbit> result = new AgentSet<>();
		AgentSet<Turtle> createResult = this.cro(number,closure,"Rabbit");
		for (Turtle t : createResult){
			if (t instanceof garden.relogo.Rabbit){
				result.add((garden.relogo.Rabbit)t);
			}
	 ...
	 */
	@ReLogoBuilderGeneratedFor("garden.relogo.Rabbit")
	public AgentSet<garden.relogo.Rabbit> createOrderedRabbits(int number) {
		return createOrderedRabbits(number,null);
	}
	...
	public AgentSet<garden.relogo.Rabbit> rabbits(){
		...
		for (Object e : this.getContext().getObjects(garden.relogo.Rabbit.class)) {
	...
	public AgentSet<garden.relogo.Rabbit> rabbitsOn(Patch p){
		...
		for (Turtle t : Utility.getTurtlesOnGridPoint(p.getGridLocation(),this,"rabbit")){
			if (t instanceof garden.relogo.Rabbit)
			result.add((garden.relogo.Rabbit)t);
		}
		return result;
	...
	public AgentSet<garden.relogo.Rabbit> rabbitsOn(Turtle t){
		AgentSet<garden.relogo.Rabbit> result = new AgentSet<garden.relogo.Rabbit>();						
		for (Turtle tt : Utility.getTurtlesOnGridPoint(Utility.ndPointToGridPoint(t.getTurtleLocation()),this,"rabbit")){
	...
	public AgentSet<garden.relogo.UserTurtle> createUserTurtles(int number, Closure closure) {
	 ...
	 */
	@ReLogoBuilderGeneratedFor("garden.relogo.UserTurtle")
	public AgentSet<garden.relogo.UserTurtle> createUserTurtles(int number) {
	 ...
	 */
	@ReLogoBuilderGeneratedFor("garden.relogo.UserTurtle")
	public AgentSet<garden.relogo.UserTurtle> createOrderedUserTurtles(int number, Closure closure) {
		AgentSet<garden.relogo.UserTurtle> result = new AgentSet<>();
	 ...
	 */
	@ReLogoBuilderGeneratedFor("garden.relogo.UserTurtle")
	public AgentSet<garden.relogo.UserTurtle> createOrderedUserTurtles(int number) {
	...
	public AgentSet<garden.relogo.UserTurtle> userTurtles(){
		...
		for (Object e : this.getContext().getObjects(garden.relogo.UserTurtle.class)) {
	...
	public AgentSet<garden.relogo.UserLink> userLinks(){
		...
		for (Object e : this.getContext().getObjects(garden.relogo.UserLink.class)) {
	...
	public garden.relogo.UserLink userLink(Number oneEnd, Number otherEnd) {
		return (garden.relogo.UserLink)(this.getNetwork("UserLink").getEdge(turtle(oneEnd),turtle(otherEnd)));
	}
